...
def train(train_loader, classifier, criterion, optimizer, epoch, print_freq=10):
    ...
    for idx, (features, labels) in enumerate(train_loader):
        ...
        # breakpoint()
        correct = (F.sigmoid(output) > 0.5).long().view(-1).eq(labels.view(-1))
...
def get_linear_acc(ftrain, ltrain, ftest, ltest, n_cls, epochs=10, args=None, classifier=None, print_ret=True, normed=False):
    ...
    for epoch in range(opt.start_epoch + 1, opt.epochs + 1):
        ...
        print('epoch {}, cur acc: {:.2f}, best accuracy: {:.2f}'.format(epoch, val_acc, best_acc))
    if print_ret:
        print(f'{acc:.2f}\t{best_acc:.2f}', end='\t')

    # classifier.load_state_dict(best_state)
    return best_acc.item(), (classifier, cluster2label, label2cluster, best_preds), loss_train
