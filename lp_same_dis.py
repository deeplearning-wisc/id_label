...
parser.add_argument('--ood_name', default='svhn', type=str)
...
def ood_feat_loader(name):
    if name == 'svhn':
        ...
        aux_out_data = dset.CIFAR100('/nobackup-slow/dataset/my_xfdu/cifarpy/', train=False,
                                      transform=get_aug(train=False, train_classifier=False, **args.aug_kwargs))
    elif name == 'cifar10':
        aux_out_data = dset.CIFAR10('/nobackup-slow/dataset/my_xfdu/cifarpy/', train=False,
                                     transform=get_aug(train=False, train_classifier=False, **args.aug_kwargs))
    else:
    ...
    rng.shuffle(idx)
    train_len = int(0.75 * len(aux_out_data))
    train_idx = idx[:train_len]
    test_idx = idx[train_len:]

    train_ood_data = torch.utils.data.Subset(aux_out_data, train_idx)
    test_ood_data = torch.utils.data.Subset(aux_out_data, test_idx)

    aux_out_data_loader = torch.utils.data.DataLoader(train_ood_data, batch_size=args.train.batch_size, shuffle=True, num_workers=2)
    features_train_n, ltrain_n = feat_extract(aux_out_data_loader, ood=1, layer=args.layer)

    aux_out_data_loader = torch.utils.data.DataLoader(test_ood_data, batch_size=args.train.batch_size, shuffle=False,
                                                      num_workers=2)
    features_test_n, ltest_n = feat_extract(aux_out_data_loader, ood=1, layer=args.layer)

    return features_train_n, ltrain_n, features_test_n, ltest_n
...
features_train_out, ltrain_out, features_test_out, ltest_out = ood_feat_loader(args.ood_name)
...
clf_known.eval()
# print(losses_train)
pred_scores = torch.sigmoid(clf_known(torch.from_numpy(np.concatenate([ftest_in, ftest_out],0)).cuda()))
...
print_measures(measures[0], measures[1], measures[2], args.ood_name)


##knn-score#
import faiss

index = faiss.IndexFlatL2(ftrain_in.shape[1])
index.add(ftrain_in)
for K in [25]:

    D, _ = index.search(ftest_in, K)
    scores_in = -D[:,-1]
    all_results = []
    all_score_ood = []
    # for ood_dataset, food in food_all.items():
    D, _ = index.search(ftest_out, K)
    scores_ood_test = -D[:,-1]
    all_score_ood.extend(scores_ood_test)
    measures = get_measures(scores_in,
                            all_score_ood, plot=False)
    print_measures(measures[0], measures[1], measures[2], args.ood_name)
